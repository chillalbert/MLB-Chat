          ...
          <div>
            <div className="flex items-center space-x-2">
              ...
              <div className="flex items-center space-x-1 bg-green-50 px-1.5 py-0.5 rounded">
                ...
                <span className="text-green-600 text-[9px] font-black uppercase">Live</span>
              </div>
            </div>
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-6">
              ...
              {room.members.map((member) => (
                <div key={member.id} className="flex items-center justify-between group p-2.5 rounded-xl hover:bg-white transition duration-200 border border-transparent hover:border-gray-100">
                  <div className="flex items-center space-x-3 overflow-hidden">
                    <div className={`flex-shrink-0 w-9 h-9 rounded-lg flex items-center justify-center text-white font-black text-sm ${member.role === 'admin' ? 'mlb-gradient' : 'bg-gray-300'}`}>
                  ...
                  {isAdmin && member.id !== user.id && (
                    <button 
                      onClick={() => {
                        if(confirm(`Remove ${member.name} from dugout?`)) onRemoveMember(member.id);
                      }} 
          ...
          <section>
            <div className="flex items-center space-x-2 px-2 mb-3">
              ...
              <h4 className="text-[10px] font-black text-gray-400 uppercase tracking-widest text-orange-600">Statcast</h4>
            </div>
            <div className="space-y-2">
              {room.leaderboard.length === 0 ? (
                <div className="text-[10px] font-bold text-gray-400 italic px-2">No records this inning...</div>
              ) : (
                room.leaderboard.map((rec, i) => (
                  <div key={i} className="bg-white p-2.5 rounded-xl border border-gray-100 shadow-sm flex items-center justify-between">
                    <div className="overflow-hidden pr-2">
                      ...
                      <div className="text-[8px] text-gray-400 font-bold uppercase">{rec.type === 'reaction' ? 'Fastball' : 'Derby'}</div>
                    </div>
      ...
      <div className="flex-1 flex flex-col relative bg-white">
        <header className="px-6 py-4 border-b border-gray-100 flex items-center justify-between bg-white/95 backdrop-blur-md sticky top-0 z-20">
            ...
            <div className="flex-1 overflow-hidden">
              <div className="flex items-center space-x-2 group">
                {isEditingName ? (
                  <div className="flex items-center space-x-2">
                    <input 
                      className="text-xl font-black text-gray-900 tracking-tighter uppercase italic bg-gray-50 border border-gray-200 rounded px-2 py-1 outline-none" 
                      value={newRoomName} 
                  ...
                  <>
                    <h1 className="text-2xl font-black text-gray-900 tracking-tighter uppercase italic truncate">{room.name}</h1>
                    {isAdmin && (
                      <button onClick={() => setIsEditingName(true)} className="p-1 text-gray-300 opacity-0 group-hover:opacity-100 hover:text-blue-600 transition">
              ...
              <div className="flex items-center space-x-2 mt-0.5">
                <span className="text-[9px] font-black text-gray-400 uppercase tracking-widest">Room Code: {room.code}</span>
              </div>
            </div>
          </div>
          <div className="hidden sm:flex items-center space-x-4">
             <div className="text-right">
               ...
               <div className="text-[8px] text-[#BA0C2F] font-black uppercase tracking-widest">{isAdmin ? 'Team Manager' : 'Active Player'}</div>
             </div>
        ...
        <div className="flex-1 overflow-y-auto p-4 md:p-6 space-y-6 bg-white">
          {room.messages.length === 0 && (
            <div className="flex flex-col items-center justify-center h-full text-center p-10 opacity-20 grayscale">
               <svg className="w-20 h-20 mb-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1"><path d="M17 8h2a2 2 0 012 2v6a2 2 0 01-2 2h-2v4l-4-4H9a1.994 1.994 0 01-1.414-.586m0 0L3 20v-4a2 2 0 01-2-2V6a2 2 0 012-2h10a2 2 0 012 2v3.586a1 1 0 01-.293.707l-2 2A1 1 0 0113 13H5" /></svg>
               <p className="font-black uppercase tracking-widest text-sm">Quiet inning... start chatting!</p>
            </div>
          )}
            ...
            if (isSystem) {
              return (
                <div key={msg.id} className="flex justify-center my-6">
                  <span className="text-[9px] font-black text-blue-700 bg-blue-50 border border-blue-100 px-6 py-2.5 rounded-full uppercase tracking-widest text-center max-w-sm shadow-sm">
                    {msg.content}
                  </span>
                </div>
              );
            ...
            return (
              <div key={msg.id} className={`flex ${isMe ? 'justify-end' : 'justify-start'}`}>
                <div className={`max-w-[85%] md:max-w-[70%] flex ${isMe ? 'flex-row-reverse' : 'flex-row'} items-start`}>
                  <div className={`flex-shrink-0 w-8 h-8 rounded-lg flex items-center justify-center text-white text-[10px] font-black shadow-sm mt-1 ${isMe ? 'ml-3 bg-[#BA0C2F]' : 'mr-3 bg-[#002D72]'}`}>
                    {msg.senderName.charAt(0)}
                  </div>
                  <div className={`flex flex-col ${isMe ? 'items-end' : 'items-start'}`}>
                    {!isMe && <div className="text-[9px] font-black text-gray-400 mb-1 ml-1 uppercase tracking-wider">{msg.senderName}</div>}
                    <div className={`px-4 py-3 rounded-2xl text-sm font-medium leading-relaxed shadow-sm break-words ${isMe ? 'bg-gray-900 text-white rounded-tr-none' : 'bg-gray-100 text-gray-800 rounded-tl-none border border-gray-100'}`}>
                      {msg.content}
                    </div>
        ...
        <footer className="p-4 md:p-6 bg-white border-t border-gray-100">
          <form onSubmit={handleSend} className="flex items-center space-x-3 max-w-4xl mx-auto">
            <input 
              type="text" 
              className="flex-1 bg-gray-50 border-2 border-gray-100 rounded-2xl px-5 py-4 text-sm font-medium focus:bg-white focus:border-[#002D72] focus:ring-0 outline-none transition" 
            ...
            <button 
              ...
              className="mlb-gradient text-white p-4 rounded-2xl hover:opacity-90 transition disabled:opacity-30 shadow-xl shadow-blue-900/10 active:scale-95"
            >
