...
const ChatRoomComponent: React.FC<ChatRoomProps> = ({
  ...
  return (
    <div className="w-full max-w-6xl h-screen md:h-[90vh] flex flex-col md:flex-row bg-slate-900 rounded-none md:rounded-[2.5rem] shadow-2xl overflow-hidden border border-slate-800">
        ...
        <div className="p-4 safe-pt border-b border-slate-800 bg-slate-950 flex justify-between items-center">
          <div className="flex bg-slate-900 p-1 rounded-xl w-full mr-2">
            <button 
              ...
              className={`flex-1 py-2 text-[10px] font-black uppercase tracking-widest rounded-lg transition-all ${sidebarTab === 'lineup' ? 'bg-slate-800 shadow-sm text-indigo-400' : 'text-slate-500'}`}
            >
              Lineup
            </button>
            <button 
              onClick={() => setSidebarTab('hall')}
              className={`flex-1 py-2 text-[10px] font-black uppercase tracking-widest rounded-lg transition-all ${sidebarTab === 'hall' ? 'bg-slate-800 shadow-sm text-indigo-400' : 'text-slate-500'}`}
            >
              Hall of Fame
            </button>
          </div>
          <button onClick={() => setShowMembers(false)} className="md:hidden p-2 text-slate-500 hover:text-slate-300">
          ...
          {sidebarTab === 'lineup' ? (
              ...
              return (
                <div key={member.id} className={`flex items-center justify-between p-3 rounded-2xl bg-slate-900 border ${isMe ? 'border-indigo-900 ring-2 ring-indigo-900/50' : 'border-slate-800'} shadow-sm`}>
                  <div className="flex items-center space-x-3 overflow-hidden">
                    <div className={`flex-shrink-0 w-10 h-10 rounded-xl flex items-center justify-center text-white font-bold text-sm ${member.role === 'admin' ? 'bg-indigo-600' : 'bg-slate-700'}`}>
            ...
            <div className="space-y-6">
              {['derby', 'heat', 'stealer'].map(game => (
                <div key={game}>
                  <h4 className="text-[9px] font-black text-slate-500 uppercase tracking-[0.2em] mb-3 ml-1">{game === 'derby' ? 'Home Run Derby (ft)' : game === 'heat' ? 'Fastest Pitch (ms)' : 'Base Stealing (s)'}</h4>
                  <div className="space-y-1.5">
                    {room.leaderboard[game as keyof typeof room.leaderboard].map((entry, idx) => (
                      <div key={entry.userId} className="flex items-center justify-between p-2.5 rounded-xl bg-slate-900 border border-slate-800">
                        ...
                        <span className="text-xs font-black text-indigo-400">{entry.score}{game === 'stealer' ? 's' : ''}</span>
                      </div>
                    ))}
                    {room.leaderboard[game as keyof typeof room.leaderboard].length === 0 && <div className="text-[10px] text-slate-600 italic p-2">No records yet</div>}
                  </div>
                </div>
              ))}
            </div>
        ...
        <div className="p-4 border-t border-slate-800 bg-slate-950 space-y-2">
          {installPrompt && (
            <button 
              ...
              className="w-full bg-slate-900 text-slate-100 font-black py-2.5 rounded-xl text-[10px] uppercase tracking-widest text-center transition hover:bg-slate-800 border border-slate-800"
            >
              📥 Download App
            </button>
          )}
      ...
      <div className="flex-1 flex flex-col bg-slate-900">
        <header className="px-6 py-5 safe-pt border-b border-slate-800 flex items-center justify-between sticky top-0 bg-slate-900/95 backdrop-blur-md z-20">
          <div className="flex items-center space-x-4 overflow-hidden">
            ...
            <div className="flex-1 overflow-hidden">
              ...
              <div className="text-[10px] font-bold text-slate-500 uppercase tracking-widest">Access Code: {room.code}</div>
            </div>
          </div>
          <div className="flex items-center space-x-2">
            <button onClick={() => setActiveGame('derby')} className="hidden sm:flex items-center space-x-2 bg-slate-950 text-white px-4 py-2 rounded-xl text-[10px] font-bold uppercase tracking-widest hover:bg-black transition shadow-lg"><span>⚾ Derby</span></button>
            <button onClick={() => setActiveGame('heat')} className="hidden sm:flex items-center space-x-2 mlb-gradient text-white px-4 py-2 rounded-xl text-[10px] font-bold uppercase tracking-widest hover:opacity-90 transition shadow-lg"><span>🔥 Heat</span></button>
            <button onClick={() => setActiveGame('stealer')} className="hidden sm:flex items-center space-x-2 bg-indigo-600 text-white px-4 py-2 rounded-xl text-[10px] font-bold uppercase tracking-widest hover:bg-indigo-700 transition shadow-lg"><span>🏃 Steal</span></button>
          </div>
        </header>
            ...
            return (
              <div key={msg.id} className={`flex ${isMe ? 'justify-end' : 'justify-start'}`}>
                <div className={`max-w-[85%] flex flex-col ${isMe ? 'items-end' : 'items-start'}`}>
                  ...
                  <div className={`px-5 py-3 rounded-2xl text-[14px] shadow-sm break-words ${isMe ? 'bg-indigo-600 text-white rounded-tr-none' : isSystem ? 'bg-slate-900 border-2 border-indigo-900 text-indigo-400 font-bold italic rounded-xl' : 'bg-slate-800 text-slate-100 rounded-tl-none border border-slate-700'}`}>{msg.content}</div>
        ...
        <footer className="p-4 bg-slate-900 border-t border-slate-800 safe-pb">
          <div className="flex sm:hidden justify-center space-x-2 mb-3 overflow-x-auto pb-2">
             <button onClick={() => setActiveGame('derby')} className="bg-slate-950 text-white px-3 py-2 rounded-lg text-[8px] font-black uppercase whitespace-nowrap">⚾ Derby</button>
          ...
          <form onSubmit={handleSend} className="flex space-x-3 max-w-5xl mx-auto">
            <input type="text" className="flex-1 bg-slate-950 border-2 border-slate-800 rounded-2xl px-6 py-3 text-sm font-medium focus:bg-slate-900 focus:border-indigo-600 text-slate-100 outline-none transition-all shadow-inner" placeholder="Message the dugout..." value={inputMessage} onChange={(e) => setInputMessage(e.target.value)} />
            <button type="submit" disabled={!inputMessage.trim()} className="bg-indigo-600 text-white px-6 rounded-2xl hover:bg-indigo-700 transition shadow-xl active:scale-95 disabled:opacity-30"><svg className="w-5 h-5 rotate-90" fill="currentColor" viewBox="0 0 20 20"><path d="M10.894 2.553a1 1 0 00-1.788 0l-7 14a1 1 0 001.169 1.409l5-1.429A1 1 0 009 15.571V11a1 1 0 112 0v4.571a1 1 0 00.725.962l5 1.428a1 1 0 001.17-1.408l-7-14z" /></svg></button>
          </form>
        </footer>
      </div>
    </div>
