...
const ChatRoomComponent: React.FC<ChatRoomProps> = ({
  ...
  const onGameOver = (game: 'derby' | 'heat' | 'stealer', score: number) => {
    ...
    const msgs = {
      ...
      heat: `🔥 CAUGHT A ${score}MS FASTBALL!`,
    ...
    <div className="w-full max-w-6xl h-screen md:h-[90vh] flex flex-col md:flex-row bg-slate-900 rounded-none md:rounded-[2.5rem] shadow-2xl overflow-hidden border border-slate-800">
      ...
      `}>
        <div className="p-4 safe-pt border-b border-slate-800 bg-slate-950 flex justify-between items-center">
          <div className="flex bg-slate-900 p-1 rounded-xl w-full mr-2">
            <button 
              ...
              className={`flex-1 py-2 text-[10px] font-black uppercase tracking-widest rounded-lg transition-all ${sidebarTab === 'lineup' ? 'bg-slate-800 text-indigo-400' : 'text-slate-500'}`}
            >
              Roster
            </button>
            <button 
              onClick={() => setSidebarTab('hall')}
              className={`flex-1 py-2 text-[10px] font-black uppercase tracking-widest rounded-lg transition-all ${sidebarTab === 'hall' ? 'bg-slate-800 text-indigo-400' : 'text-slate-500'}`}
            >
              Records
            </button>
          </div>
          <button onClick={() => setShowMembers(false)} className="md:hidden p-2 text-slate-500 hover:text-white">
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-2 custom-scrollbar bg-slate-950/50">
          {sidebarTab === 'lineup' ? (
            room.members.map((member) => {
              ...
              return (
                <div key={member.id} className={`flex items-center justify-between p-3 rounded-2xl bg-slate-900 border ${isMe ? 'border-indigo-900 ring-2 ring-indigo-900/40' : 'border-slate-800'} shadow-sm`}>
                  <div className="flex items-center space-x-3 overflow-hidden">
                    <div className={`flex-shrink-0 w-10 h-10 rounded-xl flex items-center justify-center text-white font-bold text-sm ${member.role === 'admin' ? 'bg-indigo-600 shadow-lg shadow-indigo-950/50' : 'bg-slate-700'}`}>
            ...
            <div className="space-y-6">
              {['derby', 'heat', 'stealer'].map(game => (
                <div key={game}>
                  <h4 className="text-[9px] font-black text-slate-500 uppercase tracking-[0.2em] mb-3 ml-1">{game === 'derby' ? 'Derby (ft)' : game === 'heat' ? 'Fastball (ms)' : 'Steal (s)'}</h4>
                  <div className="space-y-1.5">
                    {room.leaderboard[game as keyof typeof room.leaderboard].map((entry, idx) => (
                      <div key={entry.userId} className="flex items-center justify-between p-2.5 rounded-xl bg-slate-900 border border-slate-800">
                        <div className="flex items-center space-x-2">
                          <span className={`w-5 h-5 flex items-center justify-center rounded-lg text-[10px] font-black ${idx === 0 ? 'bg-amber-900/30 text-amber-500' : 'bg-slate-800 text-slate-600'}`}>{idx + 1}</span>
                    ...
                    {room.leaderboard[game as keyof typeof room.leaderboard].length === 0 && <div className="text-[10px] text-slate-700 italic p-2">No records</div>}
                  </div>
                </div>
              ))}
            </div>
        ...
        <div className="p-4 border-t border-slate-800 bg-slate-950 space-y-2">
          ...
          <button onClick={onLogout} className="w-full text-slate-600 hover:text-rose-500 font-bold py-2 text-[10px] uppercase tracking-[0.2em] text-center transition">Logout</button>
        </div>
      </div>
      ...
      <div className="flex-1 flex flex-col bg-slate-900">
            ...
            <div className="flex-1 overflow-hidden">
              {isEditingName ? (
                ...
                <div className="flex items-center space-x-2 group">
                  <h1 className="text-2xl font-extrabold text-slate-100 truncate tracking-tight uppercase italic">{room.name}</h1>
              ...
              <div className="text-[10px] font-bold text-slate-500 uppercase tracking-widest">Dugout Code: {room.code}</div>
            </div>
          </div>
          <div className="flex items-center space-x-2">
            ...
            return (
              <div key={msg.id} className={`flex ${isMe ? 'justify-end' : 'justify-start'}`}>
                <div className={`max-w-[85%] flex flex-col ${isMe ? 'items-end' : 'items-start'}`}>
                  ...
                  <div className={`px-5 py-3 rounded-2xl text-[14px] shadow-xl break-words ${isMe ? 'bg-indigo-600 text-white rounded-tr-none' : isSystem ? 'bg-slate-900 border-2 border-indigo-900 text-indigo-400 font-black italic rounded-xl' : 'bg-slate-800 text-slate-100 rounded-tl-none border border-slate-700'}`}>{msg.content}</div>
        ...
        <footer className="p-4 bg-slate-900 border-t border-slate-800 safe-pb">
          <div className="flex sm:hidden justify-center space-x-2 mb-3 overflow-x-auto pb-2">
             <button onClick={() => setActiveGame('derby')} className="bg-slate-950 text-white px-3 py-2 rounded-lg text-[8px] font-black uppercase border border-slate-800">⚾ Derby</button>
             <button onClick={() => setActiveGame('heat')} className="mlb-gradient text-white px-3 py-2 rounded-lg text-[8px] font-black uppercase">🔥 Heat</button>
             <button onClick={() => setActiveGame('stealer')} className="bg-indigo-600 text-white px-3 py-2 rounded-lg text-[8px] font-black uppercase">🏃 Steal</button>
          </div>
